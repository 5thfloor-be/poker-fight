...
const Room: NextPage = () => {
  ...
  const me = new User("20", {
    name: "Gab",
    color: "#008000",
    role: Role.SCRUM_MASTER,
  });

  const pedro = new User("10", {
    name: "Pedro",
    color: "#ffff00",
    role: Role.DEV,
  });
  const estef = new User("30", {
    name: "Estef",
    color: "#ffc0cb",
    role: Role.DEV,
  });
  const tbo = new User("40", { name: "Tbo", color: "#ffa500", role: Role.DEV });
  const alex = new User("50", {
    name: "Alex",
    color: "#ffc0cb",
    role: Role.DEV,
  });
  const mathieu = new User("60", {
    name: "Mathieu",
    color: "#808080",
    role: Role.DEV,
  });
  const renaud = new User("70", {
    name: "Renaud",
    color: "#0000ff",
    role: Role.DEV,
  });
  const rachel = new User("80", {
    name: "Rachel",
    color: "#808080",
    role: Role.DEV,
  });
  const michou = new User("90", {
    name: "Michou",
    color: "#ff0000",
    role: Role.DEV,
  });
  const jerry = new User("100", {
    name: "Jerry",
    color: "#0000ff",
    role: Role.DEV,
  });
        ...
        <div className="row">
          <div className="col text-center text-xl-center mt-sm-5 mb-sm-5">
            <button className="btn btn-lg btn-primary">
              <h1>
                {selecedIndex != -1 ? selecedIndex : "Waiting for votes ..."}
              </h1>
            </button>
          </div>
        </div>
